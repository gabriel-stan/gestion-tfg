...
class Tfgs_asig_masivos(Tfgs_masivos):
    ...
    def upload_file_confirm(self, tfgs):
        ...
        for index, data_tfg in enumerate(tfgs):
            try:
                self.alumno_1 = self.get_or_create_alumno(data_tfg['tfg'].get('alumno_1')) if data_tfg['tfg'] \
                    .get('alumno_1') else None
                self.alumno_2 = self.get_or_create_alumno(data_tfg['tfg'].get('alumno_2')) if data_tfg['tfg'] \
                    .get('alumno_2') else None
                self.alumno_3 = self.get_or_create_alumno(data_tfg['tfg'].get('alumno_3')) if data_tfg['tfg'] \
    ...
    def get_or_create_alumno(self, alumno):
        if utils.is_email_alumno(alumno):
            ...
            if not Alumno.objects.filter(dni=alumno if alumno else None).exists():
                Alumno.objects.create_user(dni=alumno)
            try:
