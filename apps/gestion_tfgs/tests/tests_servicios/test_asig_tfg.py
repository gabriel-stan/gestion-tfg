...
class TfgServicesTests(TestCase):
    def setUp(self):
        ...
        self.otro_user_alumn1_tfg = tfg_services.insert_alumno(Alumno(username='otro_alumn1@correo.ugr.es',
                                                                      first_name='otro_alumn1',
                                                                      last_name='otro_apellidos 1'))['data']
        self.otro_user_alumn2_tfg = tfg_services.insert_alumno(Alumno(username='otro_alumn2@correo.ugr.es',
                                                                      first_name='otro_alumn2',
                                                                      last_name='otro_apellidos 2'))['data']
        self.otro_user_alumn3_tfg = tfg_services.insert_alumno(Alumno(username='otro_alumn3@correo.ugr.es',
                                                                      first_name='otro_alumn3',
    ...
    def test_asig_tfg_tfg_error(self):
        ...
        tfg = "tfg"
        result = servicios.asignar_tfg(tfg, self.user_alumn1_tfg, self.user_alumn2_tfg, self.user_alumn3_tfg)
    ...
    def test_asig_tfg_alumno_error(self):
        ...
        result = servicios.asignar_tfg(tfg, alumno1, self.user_alumn2_tfg, self.user_alumn3_tfg)
        self.assertEqual(result['status'], False)

        # Alumno 2 y 3 no pertenecen al grupo de alumnos
        result = servicios.asignar_tfg(tfg, self.user_alumn1_tfg, self.user_tutor_tfg, self.user_cotutor_tfg)
        self.assertEqual(result['status'], False)

        # Alumno 1 y 3 no pertenecen al grupo de alumnos
        result = servicios.asignar_tfg(tfg, self.user_tutor_tfg, self.user_alumn2_tfg, self.user_cotutor_tfg)
        self.assertEqual(result['status'], False)

        # Alumno 1 y 2 no pertenecen al grupo de alumnos
        result = servicios.asignar_tfg(tfg, self.user_tutor_tfg, self.user_cotutor_tfg, self.user_alumn3_tfg)
        self.assertEqual(result['status'], False)

    def test_asig_tfg_ya_asig(self):

        tfg = Tfg.objects.get(titulo=self.titulo_tfg)
        servicios.asignar_tfg(tfg, self.user_alumn1_tfg, self.user_alumn2_tfg, self.user_alumn3_tfg)
        result = servicios.asignar_tfg(tfg, self.user_alumn1_tfg, self.user_alumn2_tfg, self.user_alumn3_tfg)
        self.assertEqual(result['status'], False)
    ...
    def test_asig_tfg_alumnos(self):
        ...
        result = servicios.asignar_tfg(otro_tfg, self.user_alumn1_tfg)
        self.assertEqual(result['status'], False)

        # Alumno2 ya tiene tfg
        result = servicios.asignar_tfg(otro_tfg, self.otro_user_alumn1_tfg, self.user_alumn2_tfg)
        self.assertEqual(result['status'], False)

        # Alumno3 ya tiene tfg
        result = servicios.asignar_tfg(otro_tfg, self.otro_user_alumn1_tfg, self.otro_user_alumn2_tfg,
                                       self.user_alumn3_tfg)
        self.assertEqual(result['status'], False)

        # Nuevo Tfg y nuevos alumnos
        result = servicios.asignar_tfg(otro_tfg, self.otro_user_alumn1_tfg, self.otro_user_alumn2_tfg,
                                       self.otro_user_alumn3_tfg)
        self.assertEqual(result['status'], True)
